...
- **Expenses** - a manual, multi-account ledger with exact paise arithmetic. Set opening balances, then create credit, expense, savings, investment, transfer or budget-only sections. Rows remain pending until **Complete** and a balance-impact confirmation. Internal transfers update both accounts together; external outflows debit only the source. Completed rows remain as records, can be repeated as new pending rows, and can be reversed with an audit trail. A virtualized Activity list includes incoming transfers, corrections, timestamps and balances after each event. No bank connection or real payment is initiated. Legacy monthly credit is folded into the balance once; old allocations and saved transfers become pending rows.
...
- **Optional encrypted Google Drive sync** - note content (all types), custom templates and reminders sync on open, after edits and periodically while foregrounded. Content is encrypted before upload and the recovery key is passphrase-wrapped. Drive still receives authorization and file metadata. **Attachment binaries and book/folder structure do not yet sync.** Conflicts use last-write-wins, not collaborative merging; avoid concurrent edits to the same expense tracker on different devices. Update every device before editing a migrated ledger.
...
Remaining work includes attachment and book sync, conflict-safe cross-device editing, wider device
coverage for the beta on-device AI stack, and full release validation. External contribution buttons
