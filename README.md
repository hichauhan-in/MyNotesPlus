...
- **Daily capture and search** - share text, selected text, images or recordings into MyNotes+ from another app, then save a new note or append to an existing text note after unlocking. Import UTF-8 TXT/Markdown documents. Search includes readable table/callout content, board text and expense records, with note-type filters, matching snippets and book-name matches. Find in note offers highlighted results and next/previous navigation.
- **Editing recovery** - undo/redo for text, checklist and board edits, plus up to 20 encrypted saved versions per note. Preview and recover a version as an independent copy. Duplicate note makes separate media copies. Media referenced by another note, template or saved version is retained; abandoned files are eligible for cleanup after 48 hours.
